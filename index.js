...
import { sleep, makeErrorResponse } from './src/utils.js';          // Fonctions utiles
import{ makePath } from './src/path-parser.js';                     // Fonctions pour la création du path
import fs from 'fs';                                                // Pour lire les fichiers (PDF)
...
var watchedPath = chokidar.watch('./data/').on('all', async (event, target_path) => {
    ...
    try {
        ...
        let errObj = makeErrorResponse(sendObj, `Can't create PATH`);
    ...
    console.log(`Modification du PATH détecté, broadcast du nouveau PATH effectué au clients.`);
});
...
wss.on('connection', (ws) => {
    ...
    ws.on('message', async (msg) => {
        ...
        if (typeof request.type === 'undefined') {
            let errObj = makeErrorResponse(sendObj, 'Empty or non-existant ressource object field');
            ws.send(JSON.stringify(errObj));
            return;
        }
        ...
        switch (request.type) {
            case calendar:
                ...
                try {
                    const icsFile = await downloadCalendar(request.resource);                   // On télécharge le fichier iCAL/ICS
            ...
            case pdf:
                ...
                try {
                    ...
                    let errObj = makeErrorResponse(sendObj, `PDF file does not exists or can't be read`);
                    ws.send(JSON.stringify(errObj));
                    return;
                }
            ...
            default:
                ...
                let errObj = makeErrorResponse(sendObj, 'Wrong type request');
                ws.send(JSON.stringify(errObj));
                return;
        }
    });
